...
let rec multi_step_reduce ~top_level_context ~type_constr_context ~expr =
  ...
  | Ast.Expr.BinaryOp (op, expr1, expr2) -> (
      ...
      | ( Ast.BinaryOperator.EQ,
          Ast.Value.Constant (c1),
          Ast.Value.Constant (c2) ) ->
          Ok (Ast.Value.Constant (Ast.Constant.Boolean (Ast.Constant.equal c1 c2)))
      | ( Ast.BinaryOperator.NEQ,
          Ast.Value.Constant (c1),
          Ast.Value.Constant (c2) ) ->
          Ok (Ast.Value.Constant (Ast.Constant.Boolean (not (Ast.Constant.equal c1 c2))))
